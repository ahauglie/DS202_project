...
Which teams have the most Homeruns hit?
```{r}
dat3 <- dat
dat3%>%
  select(Team, HR) %>%
  mutate(HR = ifelse(HR == -9999, NA, HR)) %>%
  group_by(Team) %>%
  mutate(SumHR = sum(HR, na.rm = T)) -> dat3

dat4 <- select(dat3, -HR)
dat4 <- distinct(dat4)  #ROUND TO 3 digits
head(dat3, 10)

dat4$Team <- as.factor(dat4$Team)
ggplot(dat4, aes(x = `SumHR`)) + geom_bar()
```
