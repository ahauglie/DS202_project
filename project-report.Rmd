...
**Which position has the best batting average?**   
We then wanted to look at which position had the best batting average. Our assumption was the Designated Hitters would have the highest AVG, considering their only job is to hit. However, when we took a look at the data we found very surprising results. 
```{r, echo=FALSE}
dat2 <- dat
dat2%>%
  select(Pos, AVG) %>%
...
```   
From the graph we can see that when Pujols's left the Cardinals in 2010 and joined the Angels, his stats severly suffered. We mapped Albert Pujols’s career by year, looking at 5 different stats. Since Pujols is known for being a top Home run hitter in the last 20 years, we needed to include Home Runs, RBI’s,  Hits, and Extra Base Hits. With him being such a solid hitter, pitchers were scared to pitch to him, and would end up walking him, so we look at walks as well. The black vertical line on the graph represents the end of the 2010 season, when he left St. Louis and went to the Angels. This was quite interesting as you can see a large drop off in all of his stats. This is not uncommon, a small percentage of star athletes leave their home city and just don’t do as well. This was interesting to see using a graph, as it was all the rage in the media in 2011 and 2012 when Albert Pujols was not performing. Even if you take into account him just getting older, there's still a noticable drop in his performance after he switched teams. We think a big part of this drop could be because he had played his whole career in his hometown of St. Louis, and when he switched teams many of his loyal fans were angry and he didn't have the same support and love as he did with the Cardnials.   
...
Lastly, we also took at look at the 3d plots and wanted to visually represent some of the statistics in a different way. Plotting the newly aquired variables such as the RBIAB and HRAB gave us a new look on how multiple variables can affect each other. Previously mentioned, we can see that in baseball there tends to be a positive correlation. This observation stands true in adding multiple variables and representing the data in a 3D plot. 
