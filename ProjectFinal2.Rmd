...
plot3d(dat$HR,dat$AVG,dat$RBI)  # Visualizing different statistics 

ab <- dat %>% filter(Pos == "1B")



sai <- dat %>% group_by(Pos) %>% summarise(mean_avg = mean(AVG), mean_hr = mean(HR), mean_rbi = mean(RBI))
sai1 <- dat %>% group_by(Team) %>% summarise(mean_avg = mean(AVG), mean_hr = mean(HR), mean_rbi = mean(RBI))
sai1
plot3d(sai$mean_avg,sai$mean_hr,sai$mean_rbi)
plot3d(ab$HR,ab$AVG,ab$RBI)
...
plot3d(dat$Team, dat2$Pos, dat2$AVG, color = "blue")
...
lmk <- dat2 %>% group_by(Team) %>% summarise(mean_rate = mean(AVG))
lmk
...
# Random 3d to compare different questions 

#Top teams in MLB 

ab <- dat %>% filter(Pos == "1B")   #LA Dogers 
ac <- dat %>% filter(Pos == "1B", Team == "STL")  #Cardinals 
ad <- dat %>% filter(Pos == "1B", Team == "TB")   # Tampa Bay 
...
bb <- dat %>% filter(Pos == "2B", Team == "LAD")   #LA Dogers 
bc <- dat %>% filter(Pos == "2B", Team == "STL")  #Cardinals 
bd <- dat %>% filter(Pos == "2B", Team == "TB")   # Tampa Bay 
bb
cb <- dat %>% filter(Pos == "3B", Team == "LAD")   #LA Dogers 
cc <- dat %>% filter(Pos == "3B", Team == "STL")  #Cardinals 
cd <- dat %>% filter(Pos == "3B", Team == "TB")   # Tampa Bay 


plot3d(ab$AVG, ac$AVG, ad$AVG)
plot3d(bb$AVG, bc$AVG, bd$AVG)
plot3d(cb$AVG, cc$AVG, cd$AVG)
